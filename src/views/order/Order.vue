...
<template>
  <div id="order">
    ...
    <!-- 支付方式选择 -->
    <van-radio-group v-model="orderdata.radio">
		<van-cell-group title="支付方式">
			<van-cell clickable
					@click="orderdata.radio = '1'">
				<template slot="title">
			...
			<van-cell clickable @click="orderdata.radio = '2'">
				<template slot="title">
			...
			<van-cell clickable @click="orderdata.radio = '3'">
				<template slot="title">
    ...
    <van-cell-group style="margin-top: 0.6rem">
      	<van-coupon-cell :coupons="coupons" :chosen-coupon="orderdata.chosenCoupon" @click="showList = true" />
		...
		<van-cell>
			...
			<van-coupon-list :coupons="coupons"
							:chosen-coupon="orderdata.chosenCoupon"
							@change="onChange"
							@exchange="onExchange" />
			</van-popup>
			<span slot="title">使用{{integral}}积分兑换<b>{{integralToprice | moneyFormat}}</b></span>
			<van-switch v-model="orderdata.checked"
						slot="right-icon"
						@input="onInput"
						active-color="#07c160" />
		</van-cell>
    ...
    <van-cell-group style="margin-top: 0.6rem">
		...
		<van-cell title="配送费">
			<div class="money">{{orderdata.postage}}</div>
		</van-cell>
...
<script type="text/javascript">
	...
	import { createOrder,BASE_URL } from './../../serve/api/index.js'
	...
	export default {
		data () {
			return {
				orderdata:{
					address_id:'', //收货人地址ID
					radio:'1',  //支付方式
					checked:false,  //是否积分兑换
					deliveryTime:'请选择送达时间', //送达时间
					chosenCoupon:-1,  //是否使用优惠券
					postage:0, //运费
				},
		...
		computed: {
			...
			actualPrice () {
				...
				// 是否选择优惠券
				if (this.orderdata.chosenCoupon > -1) {
					return finalPrice - this.coupons[this.orderdata.chosenCoupon].value;
				} else {
					return finalPrice;
				}
			},
		...
		methods: {
			...
			onSubmit () {
				if (!this.address_name) {
				...
				} else if (this.orderdata.deliveryTime == '请选择送达时间') {
					Toast({
						message: this.orderdata.deliveryTime,
						duration: 800
					});
				} else {
					...
					let data = {
						...
						payment:parseFloat((this.actualPrice/100).toFixed(2)),  //实际支付
						postage:orderdata.postage,   //运费
						goods:goods,
						preset_time:orderdata.deliveryTime,
						ship_id:orderdata.address_id
					}
			...
			onChange (index) {
				this.showList = false;
				this.orderdata.chosenCoupon = index;
			},
			// 优惠券兑换
			onExchange (code) {
			...
			changeData () {
				...
				this.orderdata.deliveryTime = arguments[0][1];
			}
		},
		mounted () {
			// 初始化本地购物车数据
			...
			PubSub.subscribe(CHOOSE_USER_ADDRESS, (msg, data) => {
				if (msg == CHOOSE_USER_ADDRESS) {
					...
					this.orderdata.address_id = data.id;
				}
			});
		},
		beforeDestroy () {
