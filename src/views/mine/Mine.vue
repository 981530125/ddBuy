...
<template>
  <div id="mine">
    <van-nav-bar title="我的" :fixed=true :border=false class="minebar" style="height:2.5rem;" />
		<van-cell-group style="margin-top:2.4rem">
			<van-cell class='icon-link' style="background-color: #ff8097;color:#FFF;"
    ...
    <!-- 订单相关-->
		<van-cell-group>
			...
			<van-grid :border=false>
				<van-grid-item v-for="(order,index) in orderData"
							...
							:icon="order.icon"
							:text="order.title"
              				:badge="order.type != '' ?userInfo.statusnumlist?userInfo.statusnumlist[order.type]:'':''"
		...
		<van-cell-group style="margin-top:0.4rem">
			<van-cell title="我的优惠券"
						...
						is-link />
			<van-cell title="我的收货地址" icon="todo-list" is-link @click="goToPage('myAddress')" />
		</van-cell-group>
...
export default {
 	data () {
		...
		// 订单状态
		orderData: [
			{ icon: 'cart-circle-o', title: '待支付',type:'wait' },
			{ icon: 'gift-o', title: '待收货',type:'unfinish' },
			{ icon: 'smile-comment-o', title: '待评价' ,type:'unassess'},
			{ icon: 'cash-back-record', title: '售后/退款',type:'' }
		],
